// bootstrap plugin for searchable inputs
// found from https://www.webslesson.info/2022/02/bootstrap-5-select-dropdown-with-search-box-using-vanilla-javascript-php-mysql.html

function dselectUpdate(button, classElement, classToggler) {
    //console.log(`button: ${button} + classElement: ${classElement} + classToggler: ${classToggler}`);
    //console.log(button);
  ...
  function dselect(el, option = {}) {
    ...
    const searchInput = search ? `<input onkeydown="return event.key !== 'Enter'" onkeyup="dselectSearch(event, this, '${classElement}', '${classToggler}', ${creatable})" type="text" class="form-control" placeholder="Search" id="dselect-search" autofocus>` : ''
    ...
    function selectedText(options) {
      const selectedOption = options[options.selectedIndex]
      dselectSelectedValue = selectedOption;
    ...
    function createDom() {
      ...
      <div class="dropdown ${classElement} ${additionalClass}">
        <button class="${classToggler} text-start ${!el.multiple && clearable ? classTogglerClearable : ''}" data-dselect-text="${!el.multiple && selectedText(el.options)}" type="button" data-bs-toggle="dropdown" onclick="focusSearch(this)" id="dselect-selection" aria-expanded="false"${autoclose}>
  ...
  function focusSearch(element) {
    $(element).next().find('#dselect-search').focus();
  }

  function setDSelectToID(selectElement, id) {
    const btn = selectElement.next().find('#dselect-selection').click()
    const listItems = btn.next().find('.dselect-items').children('.dropdown-item');

    for(const item of listItems) {
        if($(item).attr('data-dselect-value') == id) {
            item.click();
            break;
        }
    }
